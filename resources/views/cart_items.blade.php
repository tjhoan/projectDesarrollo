    ...
    @foreach ($cart->items as $item)
        <div id="cart-item-{{ $item->id }}" class="flex items-center justify-between mb-4">
            <div class="flex items-center">
                <img src="{{ Str::startsWith($item->product->images->first()->image_path, 'http') ? $item->product->images->first()->image_path : Storage::url($item->product->images->first()->image_path) }}" 
